...
const pathConfig = new PathConfig(0.8, 2, 0.75, 0.9);
...
test('Turn in place path right', () => {
	...
	const path = new Path(waypoints, pathConfig);
	basicPathCheck(path);
	expect(path.coords[0].angle).toBeCloseTo(waypoints[0].angle, 0.05);
	expect(path.coords[path.coords.length - 1].angle).toBeCloseTo(waypoints[1].angle, 0.05);
	for (let i = 0; i < path.sourceSetpoints.length; i++) {
		expect(path.leftSetpoints[i].position).toBeLessThanOrEqual(-path.rightSetpoints[i].position);
	}
});
